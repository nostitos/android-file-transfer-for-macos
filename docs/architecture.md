...
Before opening a session on macOS, the main process inspects the selected phone's MTP interface in IOKit. If Apple `ptpcamerad` is the verified exclusive owner, the app performs one bounded handoff: it releases only that same-user daemon, then polls IOKit until the kernel drops the exclusive claim for up to 1.5 seconds before starting its MTP worker. The ownership check is scoped by VID, PID, and serial when available and never terminates Photos, Image Capture, or unrelated processes. If the claim is still busy, the attempt stops and reports `usb-busy`. It does not escalate privileges or reset the phone USB device.

`ptpcamerad` only runs while some Mac app holds an Image Capture device session, and launchd brings it straight back when a client still wants the phone. Preview is a common, invisible example: it keeps a device browser open with no windows showing, so `ptpcamerad` reconnects to the phone every couple of seconds, fails, and resets the phone each time. Under that storm the phone stops answering `OpenSession` for everyone, including this app, which would otherwise report only "still trying". When `ptpcamerad` reappears with a new PID within two minutes of a release, or an attempt ends in `other-app-owns-usb` (or in `phone-not-responding` right after a release), the main process reads the last 45 seconds of `ptpcamerad` entries from the unified log (`/usr/bin/log show`, at most once per 20 seconds) and takes the client bundle named in its `requestStart` lines. That app name is exposed as `usbOwnerApp` on status and inventory results for three minutes; the gate then says "Quit Preview to free your phone", the pill reads "Quit Preview", and the app connects on its own once the phone answers again. The name is cleared as soon as an MTP session opens.
